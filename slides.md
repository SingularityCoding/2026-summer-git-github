...
<div class="text-xl font-bold">Windows PowerShell</div>
<div class="mt-3 opacity-75">Windows 同学可使用，涉及文件写入时看这一组</div>
</div>
<div class="p-4 border border-main rounded">
<div class="text-xl font-bold">macOS + Windows Git Bash</div>
<div class="mt-3 opacity-75">Windows 默认推荐 Git Bash；Linux 可自行参考这一组</div>
</div>
</div>

<div class="mt-8 text-lg opacity-80">
Git / gh 命令大多两组通用；差异主要在创建文件、追加内容、查看隐藏文件这类 shell 操作。
</div>

<!--
先统一课堂口径。Windows 默认推荐 Git Bash，因为它和 macOS 终端命令更接近；
但讲义会保留 PowerShell 对照。Linux 不作为本课单独设计的支持路线，能使用
Linux 的同学通常可以自行参考 Bash 组。
-->
...
课堂上还是优先按 <code>mkdir</code>、<code>cd</code>、<code>git init</code> 三步走——更容易建立"在哪个目录操作，Git 就管哪个目录"的直觉
</div>
...
# Windows PowerShell
git switch -c conflict-a
Set-Content -Path foobar.txt -Value @("hello from conflict-a", "...")
git commit -am "Update greeting on conflict branch"

git switch main
Set-Content -Path foobar.txt -Value @("hello from main", "...")
git commit -am "Update greeting on main"
```
