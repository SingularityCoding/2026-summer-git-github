...
- `assets/diagrams/<section>/*.mmd` — mermaid sources, one per diagram-worthy moment: mostly `gitGraph` for commit-topology moments (branch creation, merges, conflicts, fork/upstream), but other mermaid diagram types are fair game where they fit better — e.g. `05-recap-three-layers.mmd` uses a `flowchart` for the working-directory/staging-area/local-repository model, which isn't a commit graph at all. Rendered to `.svg` via `tools/render-diagrams.sh`. `gitGraph` diagrams use short descriptive commit labels, not real hashes, so they don't need regenerating when example output changes.
